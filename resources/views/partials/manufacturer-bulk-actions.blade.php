<div id="{{ (isset($id_divname)) ? $id_divname : 'manufacturersBulkEditToolbar' }}" style="min-width:400px">
    <form
            method="POST"
            {{-- if we end up adding more than a single action we'll want to rethink this --}}
            {{-- ...basically should end up being a big component, but felt too big an undertaking for this PR --}}
            action="{{ route('manufacturers.bulk.delete') }}"
            accept-charset="UTF-8"
            class="form-inline"
            id="{{ (isset($id_formname)) ? $id_formname : 'manufacturersBulkForm' }}"
    >
        @csrf
