<div id="{{ (isset($id_divname)) ? $id_divname : 'suppliersBulkEditToolbar' }}" style="min-width:400px">
    <form
            ...
            class="form-inline"
            id="{{ (isset($id_formname)) ? $id_formname : 'supplierBulkForm' }}"
    >
        @csrf

        {{-- The sort and order will only be used if the cookie is actually empty (like on first-use) --}}
        <input name="sort" type="hidden" value="suppliers.id">
        ...
        <button class="btn btn-primary" id="{{ (isset($id_button)) ? $id_button : 'bulkSupplierEditButton' }}"
